<?php
$test = new Socket('127.0.0.1',9988);
...
Class Socket{
	...
	function run(){
       while (true){
            ...
            foreach ($arr as $sock){
                ...
                if($sock == $this->master){
                    //如果有新的client（客户端）连接进来

                    //接受一个socket连接
                    $client = socket_accept($this->master);

                    //给新新进来的socket生成一个唯一id
                    $key = uniqid();//uniqid函数基于以微秒计的当前时间，生成一个唯一的 ID
                    ...
                    //否则1.为client断开socket连接，2.client发送信息 client是客户端
                    ...
                    if($len < 7){
                        //给该client的socket进行断开操作，并在$this->users和$this->sockets里面进行删除
                        $this->send2($k);
                        continue;
                    }
    ...
    */
	...
	function WebSocket($address,$port){
		...
		$this->re_log('Server Started : '.date('Y-m-d H:i:s'));
        $this->re_log('Listening on   : '.$address.' port '.$port);
		return $server;
	}
    ...
    function send1($k,$ar,$key='all'){
        ...
        $str = $this->code(json_encode($ar));//对发送信息进行编码处理

        if($key == 'all'){
            //面对所有在线用户发送信息
            $users = $this->users;
            if($ar['type'] == 'add'){
                //type等于add说明是新加的client
                $ar['type']='madd';
